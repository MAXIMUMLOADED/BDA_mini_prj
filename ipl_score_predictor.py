...
with st.expander("Description"):
    ...
    To make sure the model results accurate score and some reliability the minimum no. of current overs 
...
# Dummy functions for additional predictions
def get_match_winner_probability():
    # Dummy probability based on batting performance
    return np.random.uniform(0.5, 1.0)  # Random probability between 50% to 100%

def get_accuracy():
    # Assuming a dummy accuracy value for demonstration
    return 85.0  # 85% accuracy

def get_remaining_wickets(total_wickets, current_wickets):
    return total_wickets - current_wickets

def get_required_run_rate(total_runs, overs_left):
    return total_runs / overs_left if overs_left > 0 else 0
...
if st.button('Predict Score'):
    ...
    total_wickets = 10  # In cricket, a team can have a maximum of 10 wickets
    remaining_wickets = get_remaining_wickets(total_wickets, wickets)
    overs_left = 20 - math.floor(overs)  # Total overs - current overs
    total_runs = my_prediction + 5  # Assuming the higher predicted score for calculation
    required_run_rate = get_required_run_rate(total_runs, overs_left)

    # Display additional predictions
    match_winner_probability = get_match_winner_probability()
    accuracy = get_accuracy()

    st.subheader("Match Winner Prediction")
    st.success(f"Probability of Winning: {match_winner_probability:.2%}")

    st.subheader("Accuracy of the Model")
    st.write(f"Model Accuracy: {accuracy:.2f}%")

    st.subheader("Predicted Final Score")
    st.write(f"Predicted Final Score: {total_runs}")

    st.subheader("Required Run Rate")
    st.write(f"Required Run Rate: {required_run_rate:.2f} runs per over")

    st.subheader("Remaining Wickets")
    st.write(f"Remaining Wickets: {remaining_wickets}")
